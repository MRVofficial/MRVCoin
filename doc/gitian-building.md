...
*Setup instructions for a gitian build of MRV_Coin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the MRV_Coin
...
to the MRV_Coin GitHub release page.
...
- [Building MRV_Coin](#building-mrv_coin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/mrv_coin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/mrv_coin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/mrv_coin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/mrv_coin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/mrv_coin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/mrv_coin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/mrv_coin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@mrv_coin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/mrv_coin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/mrv_coin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/mrv_coin-osx-*-build.assert
```
