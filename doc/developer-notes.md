...
- [Development guidelines](#development-guidelines)
    - [General MRV_Coin](#general-mrv_coin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
MRV_Coin uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play MRVs (tMRV)" on the test network, if you
...
MRV_Coin is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/mrv_coin-project/mrv_coin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_mrv_coin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_mrv_coin --log_level=test_suite
$ valgrind -v --leak-check=full src/mrv_coind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_mrv_coin.coverage/index.html`.
```
...
- BitcoinMiner : Generates MRVs (if wallet is enabled).
...
However, in open source software such as MRV_Coin, where everyone uses
...
pay attention to for reviewers of MRV_Coin code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: MRV_Coin uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against MRV_Coin so that they can be integrated
...
file descriptors it will cause problems with MRV_Coin's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:MRV_Coin-Project/MRV_Coin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `mrv_coin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `mrv_coin-cli` and the GUI debug console use this table to determine how to
