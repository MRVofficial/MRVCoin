...
bool CMrvStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CMrvStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CMrvStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CMrvStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CMrvStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CMrvStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CMrvStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CMrvStake::GetUniqueness() const
{
    //The unique identifier for a MRV stake is the outpoint
...
CBlockIndex* CMrvStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CMrvStake::ContextCheck(int nHeight, uint32_t nTime)
{
