...
#include "qt/mrv_coin/forms/ui_snackbar.h"
#include "qt/mrv_coin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(MRV_CoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &MRV_CoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
