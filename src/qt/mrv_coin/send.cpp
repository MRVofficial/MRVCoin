...
#include "qt/mrv_coin/forms/ui_send.h"
#include "qt/mrv_coin/addnewcontactdialog.h"
#include "qt/mrv_coin/qtutils.h"
#include "qt/mrv_coin/sendchangeaddressdialog.h"
#include "qt/mrv_coin/optionbutton.h"
#include "qt/mrv_coin/sendconfirmdialog.h"
#include "qt/mrv_coin/myaddressrow.h"
#include "qt/mrv_coin/guitransactionsutils.h"
...
SendWidget::SendWidget(MRV_CoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-mrv");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
		...
		auto mrvAdd = DecodeDestination(address.toStdString(), isStakingAddr);

		if (!IsValidDestination(mrvAdd) || isStakingAddr) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(mrvAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
