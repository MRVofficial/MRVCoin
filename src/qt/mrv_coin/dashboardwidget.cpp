...
#include "qt/mrv_coin/dashboardwidget.h"
#include "qt/mrv_coin/forms/ui_dashboardwidget.h"
#include "qt/mrv_coin/sendconfirmdialog.h"
#include "qt/mrv_coin/txrow.h"
...
DashboardWidget::DashboardWidget(MRV_CoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareMrv, "square-chart-mrv");
    setCssProperty(ui->labelMrv, "text-chart-mrv");
    ...
    setCssProperty(ui->labelAmountMrv, "text-stake-mrv-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &MRV_CoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &MRV_CoinGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &MRV_CoinGUI::openFAQ);
    ...
    // update the display unit, to not use the default ("MRV")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            mrv = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalMrv += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesMrv.append(mrv);

        int max = std::max(mrv, mrv);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesMrv);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalMrv > 0) {
        setCssProperty(ui->labelAmountMrv, "text-stake-mrv");
    } else {
        setCssProperty(ui->labelAmountMrv, "text-stake-mrv-disable");
    }
    forceUpdateStyle({ui->labelAmountMrv});
    ui->labelAmountMrv->setText(GUIUtil::formatBalance(chartData->totalMrv, nDisplayUnit));
