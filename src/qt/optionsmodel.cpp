...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::MRV);
    ...
    if (settings.contains("nAnonymizeMRV_CoinAmount") || reset)
        SoftSetArg("-anonymizemrv_coinamount", settings.value("nAnonymizeMRV_CoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mrv_coin.cpp during shotdown to also remove the window positions
