...
#include "qt/mrv_coin/splash.h"
#include "qt/mrv_coin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("mrv_coin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in mrv_coin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in mrv_coin.qrc)
...
/** Class encapsulating MRV_Coin startup and shutdown.
...
/** Main MRV_Coin application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (MRV_CoinGUI)
...
{
    window = new MRV_CoinGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &MRV_CoinGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &MRV_CoinGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(MRV_CoinGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(MRV_CoinGUI::DEFAULT_WALLET);
        ...
        // MRV_Coin: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &MRV_CoinGUI::handlePaymentRequest);
        connect(window, &MRV_CoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. MRV Coin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(mrv_coin_locale);
    Q_INIT_RESOURCE(mrv_coin);
    ...
    /// 6. Determine availability of data directory and parse mrv_coin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("MRV Coin"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("MRV Coin"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // mrv_coin: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("MRV Coin didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
