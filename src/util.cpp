...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\MRV_Coin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\MRV_Coin
// Mac: ~/Library/Application Support/MRV_Coin
// Unix: ~/.mrv_coin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MRVCoin";
#else
    fs::path pathRet;
...
{
    fs::path pathConfigFile(GetArg("-conf", "mrv_coin.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty mrv_coin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override mrv_coin.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", "mrv_coind.pid"));
