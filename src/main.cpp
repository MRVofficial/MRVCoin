...
#if defined(NDEBUG)
#error "MRV_Coin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in umrv) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateMRVSupply(int nHeightStart)
{
	AssertLockHeld(cs_main);
	...
	uiInterface.ShowProgress(_("Recalculating MRV supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating MRV supply..."), percent);
		}
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight <= last_pow_block && nHeight >= 1) {
		nSubsidy = 20000 * COIN;
    } else if (nHeight <= 1500 && nHeight > last_pow_block) {
        nSubsidy = 1 * COIN;
    } else if (nHeight <= 10000 && nHeight > 1500) {
        nSubsidy = 20 * COIN;
        ...
        nSubsidy = 425 * COIN;
	} else if (nHeight <= 2421008 && nHeight > 750000) {
        nSubsidy = 450 * COIN;
    } else if (nHeight <= 2421016 && nHeight > 2421009) {
        nSubsidy = 50 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
{
    util::ThreadRename("mrv_coin-scriptch");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> mrvInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                mrvInputs.push_back(stakeIn);
        }
        const bool hasMRVInputs = !mrvInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasMRVInputs) {
                    ...
                    for (const CTxIn& mrvIn : mrvInputs)
                        if(mrvIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasMRVInputs) {
                            for (const CTxIn& stakeIn : mrvInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // MRV_Coin: We use certain sporks during IBD, so check to see if they are
